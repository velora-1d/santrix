...
<head>
    ...
    <title>Laporan Mutasi Santri - {{ tenant_name() }}</title>
    <style>
        body {
...
<body>
    ...
    <p class="subtitle">Tanggal Cetak: {{ date('d F Y') }}</p>
    
    <div class="period">
        Periode: {{ $tanggalMulai == 'Awal' ? 'Awal' : date('d F Y', strtotime($tanggalMulai)) }} 
        s/d 
        {{ $tanggalSelesai == 'Sekarang' ? 'Sekarang' : date('d F Y', strtotime($tanggalSelesai)) }}
    </div>
    ...
    @if($mutasi->count() > 0)
                ...
                @foreach($mutasi as $index => $m)
                    <tr>
                        ...
                        <td>
                            ...
                            @elseif($m->jenis_mutasi == 'pindah_kelas')
        ...
        <div class="summary">
            ...
            <p><strong>Total Mutasi:</strong> {{ $mutasi->count() }}</p>
            <p><strong>Santri Masuk:</strong> {{ $jenisCounts['masuk'] }}</p>
            <p><strong>Santri Keluar:</strong> {{ $jenisCounts['keluar'] }}</p>
            <p><strong>Pindah Kelas:</strong> {{ $jenisCounts['pindah_kelas'] }}</p>
            <p><strong>Pindah Asrama:</strong> {{ $jenisCounts['pindah_asrama'] }}</p>
        </div>
    @else
